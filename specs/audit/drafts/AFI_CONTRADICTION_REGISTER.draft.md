...
| 14 | `afi-config` | Mongo-only | P1 | No analytics/market plane and no on-chain commitment schema in the can | `grep kafka/warehouse/snowflake across repo: 0 hits; grep mintForSignal/coordinateMint/EmissionsMinted` | no |
...
| 33 | `afi-docs` | Mongo-only | P3 | Market/analytics plane (warehouse/stream feature layer) entirely absen | `grep warehouse/kafka/stream across *.md = 0 hits outside specs/; Mongo TSSD is the evidence store` | no |
