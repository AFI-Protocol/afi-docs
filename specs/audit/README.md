...
**Org delta (2026-07-16) — LangChain/LangGraph elimination:** AFI no longer uses or presents itself as based on LangChain or LangGraph, and the superseded planning set `afi-docs/archive/langgraph-migration-2025/` (14 files) was **deleted from the current tree** (git history retains it in full; commit `875fe34` and every blob remain retrievable). Following the 2026-06-25 precedent, the forensic artifacts were updated rather than left dangling: two **Info** findings that existed only to describe that archive were **retired** — `themes/J-docs-drift.json` "LangGraph-era docs are correctly archived…" (which carried the corpus's sole citation to the deleted directory) and its `recon/` twin in `AFI_RECON_CORPUS.json` records[7] / `per-repo/afi-docs.json`. One `reference_only_assumptions` entry in records[18] / `per-repo/afi-protocol.json` was retired because it **quoted verbatim** a line (`afi-protocol/architecture_overview.md:6`) that has since been rewritten; no replacement quote was invented. Elsewhere only contrastive clauses were stripped — every substantive P2/P3 finding, its severity, and its remaining citations are **intact** (`J-docs-drift` answers[2] retains 11 citations). Findings were **retired, never rewritten**: a finding asserting archive hygiene about a document that no longer exists would be a false evidentiary claim. This satisfies the `:57` rule below (documented in lieu of re-running extraction).
