...
**Org delta (2026-07-16) — orchestration-terminology elimination:** AFI's orchestration is the deterministic DAG pipeline in `afi-reactor`, and the docs no longer describe it by comparison with any third-party framework. The superseded pre-beta planning archive under `archive/` (14 files, introduced at `875fe34`) was **deleted from the current tree**; git history retains it in full, so every blob remains retrievable and no history was rewritten. Following the 2026-06-25 precedent, the forensic artifacts were updated in lockstep rather than left dangling: two **Info** findings that existed only to describe that archive were **retired** — one in `themes/J-docs-drift.json` (which carried the corpus's sole citation to the deleted directory) and its `recon/` twin in `AFI_RECON_CORPUS.json` records[7] / `per-repo/afi-docs.json`. One `reference_only_assumptions` entry in records[18] / `per-repo/afi-protocol.json` was retired because it **quoted verbatim** a line (`afi-protocol/architecture_overview.md:6`) that this pass rewrote; no replacement quote was invented. Elsewhere only contrastive clauses were stripped — every substantive P2/P3 finding and its severity is **intact** (P1 34→34; P2/P3 unchanged in every file). Citations were pruned, not merely preserved: `J-docs-drift` answers[2] retains **9** live citations, having dropped three that this pass invalidated — the deleted archive README, plus `ARCHITECTURE_STATUS.md:48` and `:52`, whose cited text (the archive section) this pass removed. No surviving citation points at deleted or shifted content. Findings were **retired, never rewritten**: a finding asserting archive hygiene about a document that no longer exists would be a false evidentiary claim. Verified afterwards: `themes`/`verify`/`reports` PASS (31/31 P0/P1), citation pool 326 → 325, zero archive-related failures across 7 seeds. This satisfies the `:57` rule below (documented in lieu of re-running extraction).
